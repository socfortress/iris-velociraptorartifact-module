...
class IrisVelociraptorartifactInterface(IrisModuleInterface):
    ...
    def register_hooks(self, module_id: int):
        ...
        if module_conf.get('velociraptorartifact_on_create_hook_enabled'):
        ...
        if module_conf.get('velociraptorartifact_on_update_hook_enabled'):
        ...
        if module_conf.get('velociraptorartifact_manual_hook_enabled'):
            status = self.register_to_hook(module_id, iris_hook_name='on_manual_trigger_ioc',
                                           manual_hook_name='Get velociraptorartifact insight')
            if status.is_failure():
                self.log.error(status.get_message())
                self.log.error(status.get_data())
    ...
    def _handle_ioc(self, data) -> InterfaceStatus.IIStatus:
        ...
        for element in data:
            ...
            if 'domain' in element.ioc_type.type_name:
                status = velociraptorartifact_handler.handle_domain(ioc=element)
            ...
            # elif element.ioc_type.type_name in etc...
                ...
                self.log.error(f'IOC type {element.ioc_type.type_name} not handled by velociraptorartifact module. Skipping')
